...
@st.cache_data
def load_csv_data():
    station_csv = BASE_DIR / "charging_station_list.csv"
    car_csv = BASE_DIR / "seoul_car_status.csv"
    gu_csv = BASE_DIR / "gu_master.csv"

    # encoding='cp949'를 추가하여 한글 깨짐 및 에러 방지
    try:
        df_station = pd.read_csv(station_csv, encoding='cp949')
        df_car = pd.read_csv(car_csv, encoding='cp949')
        df_gu = pd.read_csv(gu_csv, encoding='cp949')
    except UnicodeDecodeError:
        # 만약 파일이 utf-8-sig로 저장되어 있을 경우를 대비한 예외 처리
        df_station = pd.read_csv(station_csv, encoding='utf-8-sig')
        df_car = pd.read_csv(car_csv, encoding='utf-8-sig')
        df_gu = pd.read_csv(gu_csv, encoding='utf-8-sig')
...
with st.sidebar:
    ...
    if st.button("❓ FAQ 조회", use_container_width=True,
